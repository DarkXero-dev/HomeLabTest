...
show_completion() {
    ...
    echo -e "  ${BLUE}•${NC} Portainer UI auto-starts on first boot → ${GREEN}https://<host-ip>:9443${NC}"
    ...
    echo -e "  • Open Portainer, set admin password, deploy app stacks from its UI"
    echo -e "  • 'sudo tailscale up' to join your tailnet"
    echo -e "  • Re-login (or 'newgrp docker') so docker group membership applies"
    echo ""
}
